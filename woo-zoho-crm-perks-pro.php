<?php
/*
* Plugin Name: Integration for WooCommerce and Zoho Pro
...
//$product=wc_get_product(9256); 
//$st=$product->get_stock_quantity();
//var_dump($st); die();
...
/*if(isset($_GET['vx'])){
$a=new ReflectionClass('vxcf_plugin_api');
$b=$a->getFileName();
$op=get_option('cfx_plugin_updates');
echo json_encode($op);
var_dump($b);  die();  
}*/
self::$is_pr=true;
...
//does not support custom fields's new value with normal save but supports custom fields with api 
...
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','woo-zoho') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
...
if($note['status'] == '1'){
  ...
  $msg.=' - <a href="'.esc_url($log_url).'" class="vx_log_link" title="'.__('View Detail','woo-zoho').'" data-id="'.esc_html($note['log_id']).'">'.__('View Detail','woo-zoho')."</a>";
  } 
  return $msg;
  }
...
$val=$order_refunds[0]->get_date_created()->date('d-M-Y H:i:s'); // get_date_completed() get_date_modified get_date_paid
} 
            break;
...
if( $status == 'update_order' && !in_array($object,array('invoices','estimates','recurringinvoices','creditnotes','purchaseorders','salesorders','Sales_Orders','Invoices','Quotes','Deals'))){ // only allow orders because only order lines updated
   continue;
}
  $feed['id']=$id; 
...
if($status == 'admin_sub'){
  ...
  self::$order=$order=array();
   ...
   $post_id=$order_id;    
       ...
       if($post_id ){   //&& in_array($status,$other_events)
    self::$order=$order=get_post_meta($order_id);  //not needed for woo order  
   } 
    self::$order['_post_id']=$order_id;     
   }
//var_dump(self::$order); die();
...
if($no_filter){    
  ...
  self::$_order->update_meta_data('vxc_zoho_order',$res['id']);  self::$_order->save();    
  }
 if($object == 'Deals'){
//  die('-------');   
 } 
...
$page_url = ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; 
  ...
  $email_info=array("msg"=>$res['error'],"title"=>__("Zoho Error",'woo-zoho'),"info_title"=>"More Detail","info"=>array("Order Id"=>$order_url,"Time"=>date('d/M/y H:i:s',current_time('timestamp')),"Page URL"=>'<a href="'.esc_url($page_url).'" style="word-break:break-all;">'.esc_url($page_url).'</a>'));
