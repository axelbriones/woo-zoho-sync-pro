...
if(!empty($res['items'])){
    if(isset($_GET['vx'])){
echo json_encode($res['items']).'-------'.date('Y-M-d H:i:s',$after).'----------'.$q; //die('---------');
}
 $res['data']= array();
 foreach($res['items'] as $v){ 
   ...
   if(isset($v['stock_on_hand'])){
     ...
     if(!empty($item['item']['locations'])){
     foreach($item['item']['locations'] as $w){
        if($w['location_id'] == strtok($meta['warehouse'],'_')){ 
        $v['stock_on_hand']=floatval($w['location_'.$qty_field]);  //warehouse_stock_on_hand , warehouse_available_stock  ,  warehouse_available_for_sale_stock   , warehouse_actual_available_for_sale_stock  ,warehouse_actual_committed_stock
        /*{
...
if($cat_sel['parent_category_id'] != '-1'){
 ...
 if(!empty($pro_id) && empty($meta['sync_cat'])){ // product exist and sync cat is disabled , do not sync cat
    $cats_all=array();  
 } 
    if(!empty($cats_all)){
     $cat_id=$par_id='';   
...
if($stock === 'parent'){
    ...
    $qty_db=$parent->get_stock_quantity();    
      ...
      if($parent->get_manage_stock() == false){
          $parent->set_manage_stock(1);
      }
      $parent->save();
    }      
    }
...
<tr>
<th><label for="vx_cat"><?php esc_html_e('Sync Category','woocommerce-zoho-crm');  ?></label></th>
<td><label for="vx_cat"><input type="checkbox" id="vx_cat" name="crm[sync_cat]" value="yes" <?php if(!empty($meta['sync_cat'])){echo 'checked="checked"';} ?> > <?php esc_html_e('Sync Product Category from Zoho to WooCommerce if product already exists in WooCommerce.','woocommerce-zoho-crm');  ?></label></td>
</tr>
...
<td>  <select id="vx_item_field" name="crm[item_qty_field]" style="width: 100%">
  ...
  $cache=array(""=>"Use default Zoho field for Stock Qty","available_stock"=>"available_stock (Accounting Stock)","actual_available_stock"=>"actual_available_stock (Physical Stock)","actual_available_for_sale_stock"=>"actual_available_for_sale_stock (Physical Stock)","available_for_sale_stock"=>"available_for_sale_stock (Accounting Stock)");
  ...
  ?>
   ...
   <div class="howto"><?php esc_html_e('Stock fields work with single warehouse option.','woocommerce-zoho-crm') ?></div> 
  </td>
</tr>
